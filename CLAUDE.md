...
Read `AI_DEVELOPMENT/FIXED_INSTRUCTIONS.md` first. Its complete Japanese text was
confirmed as the fixed instruction by the user on 2026-09-13. It governs completion,
element extraction, reference selection and blind comparison. The latest explicit user
instruction takes precedence. This file preserves the product concept and implementation
brief; `PROJECT_OPERATING_PROTOCOL.md` supplies the changeable execution procedure.
Neither an older common-directive snapshot nor a technical CI gate overrides the fixed text.

Continue on the latest `codex/game2-criteria-20260911` unless remote evidence identifies
a successor. Read `AI_DEVELOPMENT/SESSION_STATE.yaml` before editing, preserve other
writers, and keep the deadline 2026-09-20T07:51:53Z. No human intervention is required.
