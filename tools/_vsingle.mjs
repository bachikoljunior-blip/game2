...
// The opening title card covers the world for its first seconds; the capture rig
// dismisses it and this check has to do the same or it photographs the card.
await p.evaluate(() => { const k = window.__kagerou; k?.menus?.skipIntro?.(); k?.menus?.resume?.(); k?.debugCam?.('hero'); }).catch(() => {});
