 ...
 * The device descriptor, coarse-pointer media query, and real tap() calls prove
 * the emulated touch path. Linux WebKit can still report maxTouchPoints as zero,
 * so that diagnostic is recorded but is not treated as hardware evidence.
...
try {
  ...
  report.emulation = {
    profile: 'iPhone SE (3rd gen) landscape',
  ...
  check(
    report.emulation.hasTouch && report.emulation.isMobile && device.coarse,
    'touch emulation and coarse pointer are active',
    JSON.stringify({ ...report.emulation, reportedMaxTouchPoints: device.maxTouchPoints, coarse: device.coarse }),
  );
