...
import { isRouteId, routeLabel } from './route-layout.js';
...
export function canLightSignal(world) {
  return world.pathCleared && isRouteId(world.routeChoice) && world.routePhase === 'rejoined' &&
    world.player.hp > 0 && world.enemies.every(enemy => enemy.hp <= 0) &&
...
export function objectiveText(world) {
  ...
  if (!remaining) return canLightSignal(world)?'E または「灯す」で、谷へ合図を送る':
    !world.routeChoice?'道は開いた。分かれた参道を通って灯へ':
      world.routePhase!=='rejoined'?`${routeLabel(world.routeChoice)}を抜け、社の前へ`:'道は開いた。社の正面の灯に近づく';
  if (world.player.z > 10 && !world.totals.kills) return '谷へ合図を送るため、鳥居の先へ';
  if (!world.routeChoice && world.enemies.find(enemy=>enemy.id==='sentinel')?.hp<=0) return '二つに分かれた参道から、社へ';
  if (world.routeChoice) return `${routeLabel(world.routeChoice)}を進む　残る剣士 ${remaining}`;
